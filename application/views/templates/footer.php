<!-- Footer Section-->
<section class="footer bg-gray">
    <div class="container">
        <div class="row footer-row">
            <div class="col-md-3 logo-tagline">
                <img class="logo" src="<?= base_url('assets'); ?>/img/logo.png" alt="logo" />
                <div class="vl mx-3"></div>
                <small class="tagline">Managing Your IT <br />
                    Complexity</small>
            </div>
            <div class="col-md-4">
                18 Parc Place Tower D, 8th Floor <br />
                Jl. Jend. Sudirman Kav.52-53 <br />
                Sudirman Central Business District (SCBD)<br />
                South Jakarta<br />
                DKI Jakarta 12190 <br />
                Phone: <a href="tel:+622139721717">+62 21 3972 1717</a> <br />
                Email: <a href="mailto:support@r17.co.id">support@r17.co.id</a>
                <br />
            </div>
            <div class="col-md-2">
                <ul class="list-unstyled mb-0">
                    <li><a href="<?= base_url(''); ?>"> Home</a> <br /></li>
                    <li><a href="<?= base_url(''); ?>about">About</a><br /></li>
                    <li>
                        <a href="<?= base_url(''); ?>#solution" class="page-scroll">Solutions</a> <br />
                    </li>
                    <li>
                        <a href="<?= base_url(''); ?>#technology" class="page-scroll">Technology</a> <br />
                    </li>
                    <li><a href="#">Insights</a></li>
                </ul>
            </div>
            <div class="col-md-3">
                <ul class="list-inline mb-0">
                    <li>
                        <a target="_blank" href="https://www.linkedin.com/company/r17kelola/"><i class="fa fa-linkedin fa-fw fa-lg"></i></a>
                    </li>
                    <li>
                        <a target="_blank" href="https://www.facebook.com/officialr17group/"><i class="fa fa-facebook fa-fw fa-lg"></i></a>
                    </li>
                    <li>
                        <a target="_blank" href="https://www.instagram.com/officialr17group/"><i class="fa fa-instagram fa-fw fa-lg"></i></a>
                    </li>
                </ul>
            </div>
        </div>
        <div class="row">
            <div class="col">
                <p class="small text-center">&copy;<?= date('Y'); ?> Rizki Tujuhbelas Kelola</p>
            </div>
        </div>
    </div>
</section>

<!-- Cookies -->
<div class="cookie-wrapper">
    <div>
        By clicking "Accept", you agree to the storing of cookies on your device
        to enhance site navigation, and help site analysis and performance.
    </div>
    <button class="btn btn-white mx-2 btn-agree">Accept</button>
    <button class="btn btn-dark mx-2 btn-decline">Not Accept</button>
</div>

<!-- jQuery-->
<script src="<?= base_url('assets/'); ?>js/jquery-1.12.3.min.js"></script>
<!-- Bootstrap Core JavaScript-->
<script src="<?= base_url('assets/'); ?>js/bootstrap.min.js"></script>
<!-- Plugin JavaScript-->
<script src="<?= base_url('assets/'); ?>js/jquery.easing.min.js"></script>
<script src="<?= base_url('assets/'); ?>js/jquery.countdown.min.js"></script>
<script src="<?= base_url('assets/'); ?>js/device.min.js"></script>
<script src="<?= base_url('assets/'); ?>js/form.min.js"></script>
<script src="<?= base_url('assets/'); ?>js/jquery.placeholder.min.js"></script>
<script src="<?= base_url('assets/'); ?>js/jquery.shuffle.min.js"></script>
<script src="<?= base_url('assets/'); ?>js/jquery.parallax.min.js"></script>
<script src="<?= base_url('assets/'); ?>js/jquery.circle-progress.min.js"></script>
<script src="<?= base_url('assets/'); ?>js/jquery.swipebox.min.js"></script>
<script src="<?= base_url('assets/'); ?>js/smoothscroll.min.js"></script>
<script src="<?= base_url('assets/'); ?>js/tweecool.min.js"></script>
<script src="<?= base_url('assets/'); ?>js/wow.min.js"></script>
<script src="<?= base_url('assets/'); ?>js/jquery.smartmenus.js"></script>

<!-- Custom Theme JavaScript-->
<script src="<?= base_url('assets/'); ?>js/cookie.js"></script>
<script src="<?= base_url('assets/'); ?>js/pheromone.js"></script>
<!-- Owl Carousel JS -->
<script src="https://cdnjs.cloudflare.com/ajax/libs/OwlCarousel2/2.3.4/owl.carousel.min.js" integrity="sha512-bPs7Ae6pVvhOSiIcyUClR7/q2OAsRiovw4vAkX+zJbw3ShAeeqezq50RIIcIURq7Oa20rW2n2q+fyXBNcU9lrw==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>
<script src="<?= base_url('assets/'); ?>js/owl-carousel.js"></script>

<!--[if lt IE 9]>
      <script src="https://oss.maxcdn.com/libs/html5shiv/3.7.0/html5shiv.js"></script>
      <script src="https://oss.maxcdn.com/libs/respond.js/1.4.2/respond.min.js"></script>
    <![endif]-->
</body>

</html>